...
public final class LocationUtils {
    ...
    private static void createLocationRequest() {
        ...
        locationRequest.setFastestInterval(LocationUtils.FASTEST_UPDATE_INTERVAL);
        locationRequest.setPriority(LocationRequest.PRIORITY_HIGH_ACCURACY);
        locationRequest.setMaxWaitTime(LocationUtils.MAX_WAIT_TIME);
    }

    @VisibleForTesting
    public static void setLocationClient(LocationClient client) {
        LocationUtils.client = client;
    }

    private static LocationClient getClient(Context context) {
        if (LocationUtils.client == null)
            LocationUtils.client = new FusedLocationClientAdapter(context);
        return LocationUtils.client;
    }
